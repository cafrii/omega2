...
두번째 단계인 최단 다리를 검색하는 부분을 개선한 버전.
...
def solve_fast(grid:list[list[int]])->int:
    ...
    log("map:\n%s", grid2str(landmap))
    log("edge cells: %s", edge_cells)

    # 2. edge cell 에서부터 영토 확장
    que = edge_cells
    ...
    while que:
        ...
        dist = distmap[cy][cx]
        if dist != prev_dist:
            log("dist %d, map:\n%s", dist, grid2str(landmap))
            # check completion condition
            if min_dist < INF:
                break
            prev_dist = dist

        # log("(%d,%d:%d)", cy, cx, dist)
        for dy,dx in deltas:
            ny,nx = cy+dy,cx+dx
            if not (0<=ny<N and 0<=nx<N): continue # out of bound
            if landmap[ny][nx] == land_id: continue # same land
            if landmap[ny][nx] > 0: # we reached to other land!
                log("    land-%d (%d,%d:%d) reached land-%d at (%d,%d:%d) -> %d",
                    land_id, cy, cx, dist, landmap[ny][nx], ny, nx, distmap[ny][nx],
                    dist + distmap[ny][nx])
                # return dist + distmap[ny][nx]
                min_dist = min(min_dist, dist + distmap[ny][nx])
                # dist 값이 변경되는 시점 까지는 계속 진행해야 함. 더 작은 값이 존재할 수 있음.
                continue

            # log("  mark (%d,%d) to land-%d, dist %d", ny, nx, land_id, dist+1)
            landmap[ny][nx] = land_id
            que.append((ny, nx))
