...
2 초 (추가 시간 없음)	128 MB	5277	2356	1948	47.282%
...
2025.9.13  9:37~10:00

----
간단한 2차원 dp. loop 전개하는 순서만 유의하면 됨.
...
A = []
for _ in range(N):
    A.append([ randint(0,N) for k in range(N) ])
A[0][0] = randint(1,5)
A[N-1][N-1] = -1
print(N)
for k in range(N): print(' '.join(map(str, A[k])))
EOF
) | $run
